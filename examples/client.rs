...
#[allow(unused_imports)]
use mbus_api::{ApiNoContext, ContextWrapperExt,
                      ApiError,
                      ApiResponse,
                      GetResponse,
                      HatResponse,
...
fn main() {
    ...
    match matches.value_of("operation") {
        ...
        */
        ...
        Some("Get") => {
            let result = core.run(client.get("ttyAMA0".to_string(), mbus_api::models::Baudrate::_2400, 48));
            println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
         },
        ...
        Some("Scan") => {
            let result = core.run(client.scan("ttyAMA0".to_string(), mbus_api::models::Baudrate::_2400));
            println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
         },
